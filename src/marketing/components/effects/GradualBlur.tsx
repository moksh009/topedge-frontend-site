...
const DEFAULT_CONFIG: BlurConfig = {
  position: 'bottom',
  strength: 2,
  height: '6rem',
  divCount: 5,
...
const mergeConfigs = (...configs: Array<Partial<BlurConfig> | GradualBlurProps>) =>
  configs.reduce<BlurConfig>((acc, c) => ({ ...acc, ...c }), { ...DEFAULT_CONFIG });
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const mobileKey = `mobile${cap}` as keyof BlurConfig;
      const tabletKey = `tablet${cap}` as keyof BlurConfig;
      const desktopKey = `desktop${cap}` as keyof BlurConfig;
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      if (config.exponential) {
        ...
        // Gentler linear stack — reads as progressive fade, not a solid fog
        blurValue = 0.055 * (progress * config.divCount + 0.65) * currentStrength;
      }

      // Wider feather bands so layers blend instead of stacking as one slab
      const p1 = Math.max(0, Math.round((increment * (i - 1) - increment * 0.35) * 10) / 10);
      const p2 = Math.round(increment * (i - 0.35) * 10) / 10;
      const p3 = Math.round(increment * i * 10) / 10;
      const p4 = Math.min(100, Math.round((increment * i + increment * 0.85) * 10) / 10);
      ...
      const divStyle: CSSProperties = {
        ...
        // Soften early layers so the fade builds gradually toward the edge
        opacity: Math.min(1, config.opacity * (0.4 + progress * 0.6)),
        transition:
