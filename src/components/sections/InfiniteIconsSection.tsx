...
const InfiniteIconsSection = () => {
  return (
    <div className="relative h-28 sm:h-48 md:h-56 lg:h-64 bg-black overflow-hidden">
      {/* Single row of images moving left */}
      <div className="absolute w-full h-full flex items-center">
        <motion.div
          className="flex gap-2 sm:gap-4 md:gap-6 lg:gap-8 items-center"
          initial={{ x: 0 }}
          animate={{
            x: [-2400, 0],
          }}
        ...
        >
          ...
          {[...images, ...images, ...images].map((image, index) => (
            <div
              key={index}
              className="relative w-24 h-24 sm:w-40 sm:h-40 md:w-48 md:h-48 lg:w-56 lg:h-56 group"
            >
              <div 
                className="absolute inset-0 rounded-md sm:rounded-xl md:rounded-2xl overflow-hidden transform transition-all duration-300 group-hover:scale-105"
              ...
              >
                ...
                <img
                  ...
                  loading="lazy"
                />
                <div className="absolute bottom-0 left-0 right-0 p-1.5 sm:p-3 md:p-4 text-white text-[10px] sm:text-sm md:text-base font-medium opacity-0 transform translate-y-4 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0">
                  {image.alt}
                </div>
              </div>
            </div>
      ...
      <div className="absolute inset-y-0 left-0 w-16 sm:w-32 md:w-40 lg:w-48 bg-gradient-to-r from-black to-transparent z-10" />
      <div className="absolute inset-y-0 right-0 w-16 sm:w-32 md:w-40 lg:w-48 bg-gradient-to-l from-black to-transparent z-10" />
    </div>
  );
};
